...
This doesn't fail to load, but unfortunately the column headers look a bit weird. Tt seems the Kraken2 'table' file does not include a column header! In this case we have to specify these ourselves...
...
If you want to standardise a single sample, you just need to specify theclassifier/ of the input file, the output file name (with a valid suffix, which will tell `taxpasta` which format to save the output), and finally the profile itself.
...
But what about the more complicated `mOTUs` case, where we have unusual comment headers, but also in this tutorial we have profiles from multiple _samples_ to be standardised?
